...
By calling abort(401), the error handler for 401 will be executed. 2. Error handler: Forbidden
...
By calling abort(403), the error handler for 403 will be executed. 3. Auth class
...
This class is the template for all authentication system you will implement. 4. Define which routes don't need authentication
...
This method must be slash tolerant: path=/api/v1/status and path=/api/v1/status/ must be returned False if excluded_paths contains /api/v1/status/. 5. Request validation!
...
If auth.current_user(request) returns None, raise the error 403 - you must use abort. 6. Basic auth
...
Otherwise, keep the previous mechanism with auth an instance of Auth. 7. Basic - Base64 part
...
You can assume authorization_header contains only one Basic. 8. Basic - Base64 decode
...
Otherwise, return the decoded value as UTF8 string - you can use decode('utf-8'). 9. Basic - User credentials
...
You can assume decoded_base64_authorization_header will contain only one :. 10. Basic - User object
...
Otherwise, return the User instance. 11. Basic - Overload current_user - and BOOM!
...
With this update, now your API is fully protected by a Basic Authentication. Enjoy! 12. Basic - Allow password with ":"
...
Improve def require_auth(self, path, excluded_paths) in api/v1/auth/auth.py by allowing _ at the end of excluded paths:
Example for excluded_paths = ["/api/v1/stat_"]:
